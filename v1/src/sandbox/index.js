...
export default class Sandbox extends Component {
    ...
    drawRegl() {
        ...
        var densities = this.computeDensities({x0,y0,x1,y1,gridSize});
        
        var nx = (x1 - x0) / gridSize; 
        var ny = (y1 - y0) / gridSize;
            ...
            var colorTexture2 = this.regl.texture({data:densities.map((e)=>[0*e,e,e*.5+100,e*.5+120]).flat(),
                ...
                */
                ...
                this.regl.frame(({deltaTime, viewportWidth, viewportHeight}) => {
                    ...
                    computeDensities = ({ x0,y0,x1,y1,gridSize})=>{
                        ...
                        var grid = d3.merge(d3.range(0, (y1 - y0)/gridSize).map(function(i) {
                            return d3.range(0, (x1 - x0)/gridSize).map(function(j) { return [x0 + j*gridSize + gridSize/2, y0+ i*gridSize + gridSize/2] });
                        }));
                        // xyPoints
