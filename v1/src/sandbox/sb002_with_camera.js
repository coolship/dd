...
export default class Sandbox extends Component {
    ...
    componentDidMount() {
        ...
        this.rockTexture2 = this.regl.texture({data:this.densities.map((e)=>[e,0*e,e,e]).flat(),shape: [this.nx,this.ny,4]})
...
*/
  ...
  void main () {
    vec4 tex = texture2D(rockTexture2, vUv*1.0).rgba;
...
#define WORLD_HEIGHT 100.0
  ...
  void main() {
    ...
    render = () => {
      ...
      var h = 1.06 * Math.min(d3.deviation(xPoints), iqr / 1.34) * Math.pow(xyPoints.length, -0.2) * 5; // factor of 3 is mine. Should increase pointspread?
      var h2 = h*h;
