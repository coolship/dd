...
class DatasetLoadingContainer extends Component {
    ...
    fetchDataset(resolve) {
        ...
        if (false) {
            //metadata.RBUFFER_url){
            /* we have processed buffer data, initialize the dataset directly */

            // this part is not necssary, but for now we're keeping around the coordinates,
            // just to initialize the kd tree. still not setting "annotations_url"
            fetch(url)
                ...
                .then(function (myJson) {
                    ...
                    //console.log(that.state.dataset); throw "hi"
                    resolve();
...
*/
        ...
        var p = new Promise(resolve => {
            ...
            .then(function (result) {
                ...
                return new Promise(resolve => {
                    if (false) {
                        //that.props.metadata.RBUFFER_url){
                        that
                            .state
                            .dataset
                            .initializeAsyncWithBuffers((loading_progress, loading_status) => {
                                that.setState({
                        ...
                        // [TODO] fix the mount / unmount issue in DNAMicroscope load which makes this
                        // check necessary.
                        ...
                        that
                            ...
                            .initializeAsync((loading_progress, loading_status) => {
                                ...
                                for (let nm of["r",
                                    ...
                                    that
                                        .props
                                        .uploadBuffer(that.props.metadata_key, that.props.metadata, nm.toUpperCase() + "BUFFER", that.state.dataset[nm]);
                                }
